...
public partial class MainWindow : Window
{
    public MainWindow()
    {
        ...
        this.Loaded += (s, e) =>
        {
            this.RbAutoMapper.Checked += OnMapperChanged;
            this.RbMapperly.Checked += OnMapperChanged;
            this.RbMapperly.IsChecked = true;

            // 默认使用 Mapperly 映射
            //ShowMappingResult(new OrderMapper());
        };
    }

    private void OnMapperChanged(object sender, RoutedEventArgs e)
    {
        IOrderMapper mapper = RbMapperly.IsChecked == true
            ? new MapperlyOrderMapper()
            : new AutoMapperOrderMapper();
        ShowMappingResult(mapper);
    }

    private void ShowMappingResult(IOrderMapper mapper)
    {
        var dto = mapper.ToDto(_order);
