...
public static class HappyEyeballsHttp
{
    ...
    public static HttpClient CreateHttpClient(string baseUrl, bool autoRedirect = true)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        Debug.WriteLine("Starting Happy Eyeballs connection to {0}:{1}", endPoint.Host, endPoint.Port);

        IPAddress[] resolvedAddresses = await GetIpsForHost(endPoint, cancellationToken).ConfigureAwait(false);
    ...
    {
        Debug.WriteLine("Trying IP {0} for happy eyeballs [{1}]", address, index);
        ...
        {
            Debug.WriteLine("Happy Eyeballs to {0} [{1}] failed: {2}", address, index, e.Message);
            socket.Dispose();
            throw;
        }
    }
    ...
    private static async Task<IPAddress[]> GetIpsForHost(DnsEndPoint endPoint, CancellationToken cancel)
    {
        ...
        IPHostEntry entry = await Dns.GetHostEntryAsync(endPoint.Host, cancel).ConfigureAwait(false);
